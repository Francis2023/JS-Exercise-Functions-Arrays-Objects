...
*/
function getName(getNameObj) {
  return `Hello, my name is ${getNameObj.name}`
}
...
function getModelYears(inventory) {
  function getYear(input,pos){
    var output = [];
    for (var i = 0; i < input.length; ++i)
      output.push(input[i][pos]);
    return output;
  }
 var result = getYear(inventory, "car_year");
 return result;
}
...
function carMaker(odometer) {
  var odo = {
    ...
    drive: function(distance){
      var newOdo = this.odometer = this.odometer + distance;
      return newOdo;
    }
  }
  return odo;
